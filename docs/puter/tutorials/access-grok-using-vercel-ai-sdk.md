# How to Use Grok with the Vercel AI SDK â xAI Provider Guide

Source: https://developer.puter.com/tutorials/access-grok-using-vercel-ai-sdk/
...
On this page[About Grok](#about-grok)[Prerequisites](#prerequisites)[Setup](#setup)[Basic Text Generation](#basic-text-generation)[Streaming](#streaming)[Why Use Puter?](#why-use-puter)[Conclusion](#conclusion)[Related](#related)

In this tutorial, you'll learn how to use Grok models with the Vercel AI SDK through Puter's OpenAI-compatible provider endpoint. No xAI API key needed â just your Puter auth token.

## About Grok

Grok is built by xAI. Grok models are designed for real-time knowledge and conversational ability. The model family includes standard Grok and faster variants for low-latency applications. Puter provides access to Grok via the Vercel AI SDK with no xAI API key needed.
...
Here's a simple text generation call using Grok 4.1 Fast:
...
You could install `@ai-sdk/xai` and use xAI's API key directly. Here's why Puter is a simpler option:

- **One API key for everything**  â no need to sign up for separate xAI, Anthropic, or OpenAI accounts. Your Puter auth token covers all providers.
...
You now have the xAI provider set up through the Vercel AI SDK via Puter â no API key needed. Swap the model string to use any [Grok model](/ai/x-ai/), or any of the hundreds of other [AI models](/ai/models/) available through Puter.
