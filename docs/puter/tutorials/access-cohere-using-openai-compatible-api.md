...
Source: https://developer.puter.com/tutorials/access-cohere-using-openai-compatible-api/
...
In this tutorial, you'll learn how to access Cohere models using the OpenAI SDK through Puter's OpenAI-compatible endpoint. No Cohere API key needed, just your Puter auth token.
...
Here's a simple chat completion using Command R+:
...
That's it. You now have access to Cohere through the OpenAI SDK via Puter, no Cohere API key needed. Swap the model string to use any [Cohere model](/ai/cohere/), from Command R to the powerful Command A, or any of the hundreds of other [AI models](/ai/models/) available through Puter.
