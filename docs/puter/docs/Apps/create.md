...
Source: https://docs.puter.com/Apps/create/
...
# ![Image](/./assets/img/function.svg)`puter.apps.create()`
...
â `https://example.com/app/index.html` 

â `http://localhost:3000/index.html` 

â `file:///path/to/index.html` 

â `ftp://example.com/index.html`
...
- `filetypeAssociations` (Array) (optional): An array of strings representing the filetypes that the app can open. Defaults to `[]`. File extentions and MIME types are supported; For example, `[".txt", ".md", "application/pdf"]` would allow the app to open `.txt`, `.md`, and PDF files.
