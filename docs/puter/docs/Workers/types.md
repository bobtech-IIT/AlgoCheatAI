...
Source: https://docs.puter.com/Workers/types/
...
# ![Image](/./assets/img/object.svg)TypeScript Types for Workers

 Websites Puter Apps Node.js Workers

---

The [`@heyputer/worker-types`](https://www.npmjs.com/package/@heyputer/worker-types) package adds TypeScript type definitions for the worker runtime â the `router`, `me`, `my`, `myself`, `puter_auth`, and `puter_endpoint` globals â plus typed route handlers with automatic `params` inference from path literals.
...
We recommend naming worker files `*.worker.js` (or `*.worker.ts`). This makes the worker-y parts of your project obvious in a file listing and lets you scope the worker globals to just those files â so `router`, `me`, etc. don't leak into the rest of your code.
...
router.get('/files/*path', ({ params }) => {
    params.path; // string â wildcard captures the remainder
});
```
...
- `request` â standard [`Request`](https://developer.mozilla.org/en-US/docs/Web/API/Request)
- `params` â route params, inferred from the path literal
- `user` / `requestor` â caller's Puter context, present only when invoked with a `puter-auth` header (e.g. via [`puter.workers.exec()`](/Workers/exec/))

[NEXT

`create()`](/./Workers/create/)[PREVIOUS
