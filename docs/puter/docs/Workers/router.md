...
Source: https://docs.puter.com/Workers/router/
...
# ![Image](/./assets/img/object.svg)`router`
...
- `user` - An object representing the user who made the request to this worker. It has a `puter` property (`user.puter`) that gives you access to that user's own Puter resources â KV, FS, AI, etc. Only available when the worker is called via [`puter.workers.exec()`](/Workers/exec/).
...
- `me` - An object representing you, the worker's owner. It has a `puter` property (`me.puter`) that gives you access to your own Puter resources â KV, FS, AI, etc.
...
- **`me.puter`**  is the  **worker context**  â your own resources, as the owner. Use this for shared application data, server-side logic, and centralized resources you control. Operations run against your account and are billed to you.
- **`user.puter`**  is the  **user context**  â the resources of the user who called the worker (available when it's executed via [`puter.workers.exec()`](/Workers/exec/), which runs it with their token). This keeps the default [User-Pays model](/user-pays-model/): each user's data stays in their own storage, billed to them, while your logic still runs server-side.

So you can mix and match within the same codebase â some endpoints reading and writing your own data (`me.puter`), others acting on the calling user's data (`user.puter`).

## Route Parameters

Sometimes part of a path isn't fixed â like a post ID or a username. You can capture these segments by prefixing them with a colon (`:`) in the route path. Each captured segment becomes a property on the `params` object, keyed by the name you gave it.
...
While a route parameter (`:name`) matches a single segment, a  **wildcard**  (`*name`) matches the rest of the path â any number of segments. Like a route parameter, the matched value is available on `params`, keyed by the name after the `*`.
...
A wildcard  **must be named**  â write `*path` (or any name you like), not a bare `*`. A pattern like `/files/*` won't act as a wildcard: with no name after it, the `*` is treated as a literal character, so the route only matches the exact path `/files/*`. The name is what gives the router a key to expose the captured value on `params`.

A common use is a catch-all route for unmatched paths â define it last so it only runs when nothing else matched (see the [404 Handler](#examples) example below).

## CORS

Every response from your worker automatically includes `Access-Control-Allow-Origin: *`, so  **simple cross-origin requests work out of the box**  â a basic `GET` or `POST` from another origin just works, no extra code.
...
The `puter-auth` header is important: when you call your worker with [`puter.workers.exec()`](/Workers/exec/), it attaches the user's Puter token in a `puter-auth` header so the worker can act on the calling user's behalf (this is what populates `user.puter`). Because that's a custom header, the browser runs a preflight first â so `puter-auth` must be listed in `Access-Control-Allow-Headers`, otherwise the preflight fails and the request never reaches your worker.

If you need different CORS rules per endpoint â for example, restricting the allowed methods or headers on a specific route â define an `OPTIONS` handler on that individual path instead of using the wildcard.
...
TypeScript Types](/./Workers/types/)[PREVIOUS
