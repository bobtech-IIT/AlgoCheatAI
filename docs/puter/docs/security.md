...
Source: https://docs.puter.com/security/
 ...
 **Need to share data across users?**  Because each user's storage lives in their own account, one user can't see another's data. To keep a single, centralized store that every user reads from and writes to, use a [Serverless Worker](/Workers/) â its code can act on the worker owner's resources, giving all users one shared backend.
...
User-Pays Model](/./user-pays-model/)[PREVIOUS
