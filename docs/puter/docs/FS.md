...
# ![Image](/./assets/img/fs.svg)Cloud Storage
 ...
 **Need to share data across users?**  Each user's files live in their own account, so one user can't read another's data. To keep centralized files that every user reads from and writes to, use a [Serverless Worker](/Workers/) â its code can act on the worker owner's resources, giving all users one shared backend.
...
`write()`](/./FS/write/)[PREVIOUS

`getDetailedAppUsage()`](/./Auth/getDetailedAppUsage/)